...
export async function salvarConfigSite(dadosBrutos: unknown): Promise<ResultadoAcao> {
  try {
    ...
    if (
      ...
      fotoPastorId === false || fotoPastoraId === false || fotoComunidadeId === false
    ) {
      return {
        ok: false,
  ...
  try {
    ...
    const schema = z.object({
      ...
      secoes: z.object({
        ...
        heroBtn2Link: z.string().trim().max(300).optional().default(""),
        mensagemLead: z.string().trim().max(400).optional().default(""),
        mensagemBtnTexto: z.string().trim().max(60).optional().default(""),
        mensagemBtnLink: z.string().trim().max(300).optional().default(""),
        appBtnTexto: z.string().trim().max(60).optional().default(""),
        appBtnLink: z.string().trim().max(300).optional().default(""),
        minisBtnTexto: z.string().trim().max(60).optional().default(""),
        minisBtnLink: z.string().trim().max(300).optional().default(""),
        celulasBtnTexto: z.string().trim().max(60).optional().default(""),
        celulasBtnLink: z.string().trim().max(300).optional().default(""),
        oracaoBtnTexto: z.string().trim().max(60).optional().default(""),
